 ...
 * <p>
 * There are 3 types of power: {@link #POWER_REDSTONE}, {@link #POWER_KINETIC} and {@link #POWER_LASER}, and 3 different
 * roles that machines can have: "producer", "consumer", "transporter". Note that you cannot transport redstone power at
 * all. */
public enum MjSimpleType implements IMjConnectorType {
    ...
    /** Some sort of transporter that converts from kinetic to laser or laser to kinetic. (This is the kinetic part of
     * the converter) */
    KN_TRANSPORTER_CONVERT(KINETIC_TRANSPORTER),

    /** Some sort of transporter that converts from kinetic to laser or laser to kinetic. (This is the laser part of the
     * converter) */
    ...
    private MjSimpleType() {
        ...
        roleType = null;
        if (ordinal() > 2) {
            // We are not POWER_REDSTONE or POWER_KINETIC or POWER_LASER
            throw new IllegalStateException("Was not POWER_KINETIC or POWER_REDSTONE or POWER_LASER! (was " + name() + ")");
        }
    }
    ...
    public boolean canSendTo(MjSimpleType to) {
        if (powerType == POWER_REDSTONE) {
            ...
            return to.powerType == POWER_REDSTONE || to.powerType == POWER_KINETIC;
        } else if (powerType == POWER_LASER) {
            return to.powerType == POWER_LASER;
        }
        return false;
    }
